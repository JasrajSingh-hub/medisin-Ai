...
class _VitalGuardScreenState extends State<VitalGuardScreen> {
  ...
  Future<void> _exportReport(Map<String, dynamic> patient) async {
    try {
      Directory? directory;
      try {
      ...
      final fileName = 'clinical_summary_${patient['name'].toString().replaceAll(' ', '_')}_$dateStr.txt';
...
Room: ${patient['room']}
Risk Level: ${patient['status']?.toString().toUpperCase() ?? 'STABLE'}
Condition: ${patient['condition'] ?? '-'}
Diagnosis: ${patient['diagnosis'] ?? 'Not specified'}
Care Mode: ${patient['care_mode'] ?? '-'}

AI DIAGNOSTICS & SYMPTOMS:
- General recovery progress stable.
- Blood oxygen normal at 98%.
- Primary symptom healing expected on timeline.

NEXT-STEP GUIDANCE:
1. Continuous vitals monitoring.
2. Review dosage limits before post-op antibiotic administration.
3. Check wound status every 12 hours.

--------------------------------------------------
CLINICAL DISCLAIMER: Generated by AI clinical assistance for coordinator reference only.
All instructions must be verified by a licensed practitioner.
''';
